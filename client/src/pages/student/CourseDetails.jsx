      ...
      <div className='flex md:flex-row flex-col-reverse gap-10 relative items-start justify-between md:px-5 px-4 md:pt-15 pt-10 text-left'>
        ...
        <div className='max-w-[650px] z-10 text-gray-500'>
          ...
          <div className='flex items-center space-x-2 pt-3 pb-1 text-sm'>
            ...
            <div className="flex">
              {[...Array(5)].map((_, i) => (<img key={i} src={i < Math.floor(calculateRating(courseData)) ? assets.star : assets.star_blank} alt="star" className="w-4 h-4 inline-block" />
              ))}
            </div>
          ...
          <p className='text-sm'>Course by <span className='text-blue-600 underline'>Techjays</span></p>
                  ...
                  <div className={`overflow-hidden transition-all duration-300 ${openSections[index] ? 'max-h-96' : 'max-h-0'}`}>
                    <ul className=' list-disc md:pl-10 pl-4 pr-4 py-2 text-gray-600'>
                      {chapter.chapterContent.map((lecture, i) => (
                        <li key={i} className='gap-2 flex items-start py-1'>
                          <img src={assets.play_icon} alt='play icon' className='w-5 h-5 h-4 mt-1' />
          ...
          <div className='pt-10 text-sm md:text-default'>
        ...
        <div className='max-w-course-card z-10 shadow-custom-card rounded-t md:rounded-none overflow-hidden bg-white min-w-[300px] sm:min-w-[420px]'>
          <img src={courseData.courseThumbnail} alt={courseData.courseTitle} className='w-full h-auto rounded-lg' />
          <div className='p-5 text-sm md:text-default'>
            <div className='flex items-center gap-2'>
